...
| 083 | La preventa cierra por reloj en el cobro: `preventa_cierra_at(fila)` y `preventa_abierta(fila)` (columnas calculadas vía PostgREST), `precio_vigente_de` devuelve `cierra_at` y `_reservar_ticket_shared` usa el mismo predicado. Falla cerrado sin `comienza_at` | **Escrito, sin aplicar** (8-sep-2026). Depende del 082. Desbloquea `sonopolisWeb` W-116 |
