...
| 080 | `pais` en `events` y `venues` (`char(2)`, `NOT NULL`, sin `DEFAULT`), backfill a `CL` e índices `(pais, comienza_at)` / `(pais)`. Completa el spec 050: los eventos scrapeados ya sabían su país, los internos no | Escrito, sin aplicar (2026-09-07) — ⚠️ **no pushear sin `sonopolisWeb/specs/w112` listo**: con el `NOT NULL` sin default, crear evento o local falla hasta que el formulario mande el país |
