...
| 080 | `pais` en `events` y `venues` (`char(2)`, `NOT NULL`, sin `DEFAULT`), backfill a `CL` e índices `(pais, comienza_at)` / `(pais)`. Completa el spec 050: los eventos scrapeados ya sabían su país, los internos no | Migración escrita y commiteada (`460b022`), **sin aplicar** — ⚠️ **no pushear sin `sonopolisWeb/specs/w114` listo** (el w112 quedó superado): con el `NOT NULL` sin default, crear evento o local falla hasta que el formulario mande el país. El `db push` va junto al deploy del W-114, migración primero. **Addenda 8-sep:** el W-114 no alcanza — esta app tampoco manda `pais` (`grep pais src/` = 0), así que crear evento/local se rompe también en móvil hasta que exista su propio spec FRONTEND |
