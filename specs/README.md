...
| 089 | Entorno local: la base reconstruida desde migraciones, antes de tocar producción | **Aplicado** (16-sep-2026) — OrbStack + `supabase start`: las 88 migraciones aplican desde cero sin un error, la cadena reconstruye. Addendum: `supabase db lint` **no es splinter** (corre `plpgsql_check`), el linter real se baja de GitHub y se corre con `psql`, y la regla del bucket necesita inyectar `splinter.public_buckets`. **56 warnings de seguridad en local contra 57 en producción** — la diferencia apunta a drift, pendiente de la contraseña de Postgres. **Bloquea a 090, 091, 092 y 093** |
