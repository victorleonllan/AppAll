...
| 081 | La fila `puntoticket` en `event_sources` (`pais = CL`, `activa` por default), con `on conflict do nothing`. Sin esquema nuevo: las columnas que la fuente va a llenar (`direccion`, `lat`, `lng`, `precio_texto`, `genero`) existen desde el 049 | Migración escrita (`20260908115505_spec_081_fuente_puntoticket.sql`), sin aplicar (8-sep-2026) — habilita `sonopolisWeb/specs/w115` (adaptador de Puntoticket); sin la fila, el pipeline multifuente del W-110 salta la fuente con error |
