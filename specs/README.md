...
| 087 | `description` en `events` | Aplicado en producción (11-sep-2026) — columna `text` nullable para que el organizador describa el evento; consumida por `sonopolisWeb` (w120/w121) |
| 088 | La reserva pendiente caduca a los 30 minutos | Aplicado en producción (16-sep-2026) — un `pending` ocupaba aforo para siempre: 23 en producción, el más viejo del 4-sep. TTL de 30 min en el conteo de aforo, `expiration_date_to` en la preferencia de MP y cancelación en `reconciliar-pagos` sólo tras preguntarle a MP. Los 23 `pending` quedaron en 0; falta ver el link de MP ya vencido |
| 089 | Entorno local: la base reconstruida desde migraciones, antes de tocar producción | **Aplicado** (16-sep-2026) — OrbStack + `supabase start`: las 59 migraciones aplican desde cero sin un error, la cadena reconstruye. Addendum: `supabase db lint` **no es splinter** (corre `plpgsql_check`), el linter real se baja de GitHub y se corre con `psql`, y la regla del bucket necesita inyectar `splinter.public_buckets`. **56 warnings de seguridad en local contra 57 en producción** — verificado contra producción el 17-sep: **también 56, sin drift**, coincidiendo función por función. **Bloquea a 090, 091, 092 y 093** |
| 090 | `search_path` fijo en las 7 funciones que quedaron sin él | **Aplicado en producción** (17-sep-2026) — por Management API, registrada como `20260917150000` (59 → 60 migraciones). Splinter en producción: **56 → 49**, −7 exactos. Cuerpos `md5` idénticos, cartelera pública 200, 0 de 30 `SECURITY DEFINER` sin `search_path`. Antes, en local: 5/5 criterios, triggers y guardas verificados en transacción revertida. Origen (16-sep-2026): Security Advisor, regla `function_search_path_mutable`. Las 7 son `security invoker`, así que el riesgo real hoy es bajo: se arregla porque es gratis (`ALTER FUNCTION`, sin tocar cuerpos), destapa los warnings que sí importan, y vuelve seguro el día que una de ellas pase a `SECURITY DEFINER` |
| 091 | El opt-in de WhatsApp deja de aceptar inserts directos del cliente | **Aplicado en producción** (17-sep-2026) — splinter **49 → 48**. `anon` que intenta un `INSERT` directo recibe `42501` (antes: 201 con `opted_in_at` a elección) y el RPC sigue funcionando; verificado en transacciones revertidas, la fila real intacta. Origen (16-sep-2026): Security Advisor, `RLS Policy Always True` sobre `whatsapp_opt_ins`. La policy de W-049 quedó huérfana cuando su propio addendum movió el alta al RPC `crear_optin_whatsapp()` (SECURITY DEFINER, salta RLS): no hay un solo `.insert()` en los dos repos, y mientras siga abierta se puede fabricar consentimiento con la anon key |
| 092 | El bucket `media` deja de ser listable por cualquiera | **Aplicado en producción** (17-sep-2026) — splinter **48 → 47**. Con la anon key real, el listado pasó de 4 entradas a 0 y `prefix=pendientes` de 2 a 0; las imágenes siguen sirviéndose por URL (200 en `perfiles/` y en `pendientes/`), las 8 policies de insert/update y los 18 objetos intactos. Origen (16-sep-2026): Security Advisor, `Public Bucket Allows Listing`. `media_select` no es lo que hace visible la cartelera (el bucket es `public`, y `/object/public/` no consulta RLS): lo único que habilita es enumerar el bucket entero, incluida `pendientes/<user_id>/`, que es arte de eventos sin publicar. Ningún cliente llama `.list()` |
| 093 | `PUBLIC` deja de ejecutar las funciones `SECURITY DEFINER` | Propuesto (16-sep-2026) — Security Advisor, `Public Can Execute SECURITY DEFINER Function`. 31 funciones `SECURITY DEFINER`, 15 con su `REVOKE` (desde el spec 046) y **16 sin él**: entre ellas `transfer_event_ownership`, llamable por `anon`. Revoke dinámico sobre `prosecdef` + grants escritos uno por uno. **El único de los cuatro que puede tumbar la app**: 27 policies sin `to` llaman a las guardas, y `events_select` es lo que lee la cartelera sin sesión. **Addendum (16-sep): no se aplica como está.** Cuatro firmas del GRANT no existen (la migración abortaría), son 14 y no 16, y el hallazgo de fondo: `event_role_of`, `is_booking_party`, `is_booking_recipient` y `es_admin_sonopolis` aceptan el usuario como argumento, así que son oráculos que saltan RLS. Decisión pendiente → spec 094. **Addendum 2 (17-sep): `REVOKE FROM PUBLIC` no revoca nada** — las funciones tienen grant explícito `anon=X/postgres` (el mismo default de Supabase que el spec 046 ya había documentado), hay que revocar `FROM PUBLIC, anon, authenticated` y la lista blanca pasa de 10 a 22 funciones. Con eso, los 4 criterios verificados en local: la cartelera sigue cargando y `transfer_event_ownership` da 42501 |
| 094 | Las guardas de permiso solo responden por uno mismo | Propuesto (17-sep-2026) — **bloquea al 093**. Cuatro guardas `SECURITY DEFINER` aceptan el usuario como argumento (`auth.uid()` es solo el default) y saltan RLS: demostrado en local que un `anon` sin sesión pregunta `event_role_of(evento, ajeno)` y recibe `owner`. Se les agrega `AND p_user = auth.uid()` — una línea por función, **cero policies tocadas**. Eliminar el parámetro exigiría `DROP CASCADE` de 7 policies por ambigüedad de sobrecarga, verificado |
| 095 | La app móvil manda el país al crear local y evento — cierra la mitad móvil de la ventana que abrió el 080. Hermano de `sonopolisWeb/specs/w114` | **Propuesto** (21-sep-2026) — **rotura viva en producción**: desde que la migración del 080 se aplicó, crear un local o un evento desde la app falla con `23502` (`grep -rn "pais" src/` devuelve cero). Selector de país en el formulario del local, catálogo ISO copiado de la web, `PAIS_POR_DEFECTO` como valor inicial sin dependencia nueva. Riesgo a verificar: `Intl.DisplayNames` puede no resolver nombres en Hermes |
| 096 | La app móvil muestra la cartelera de un solo país — espejo de `w111`+`w113` | Propuesto (21-sep-2026) — **no se implementa hasta que exista un segundo país con cartelera**. AsyncStorage hace de cookie, el filtro va en los contexts y no en las pantallas, y las pantallas privadas ("mis eventos", "mis ventas", "mi local") **no** se filtran por país. Depende del 095 |
| 097 | La fuente `donboleton` en `event_sources` — primera fila del esquema con un país que no es Chile. Habilita `sonopolisWeb/specs/w152` | **Aplicado en producción** (21-sep-2026) — `20260921211833`. Antes del push no había migraciones ajenas pendientes (62 = 62). Corrida real del pipeline: 35 conciertos de Ciudad Juárez y Chihuahua escritos con `pais = 'MX'`, 0 errores. La cartelera pública **no cambió**: `MX` no está en `PAISES`, así que los eventos entran pero no se muestran — probado en local con el catálogo extendido |
| 098 | La fuente `startickets` en `event_sources` — segunda fuente mexicana, pedida por Victor sobre la vista de Chihuahua. Habilita `sonopolisWeb/specs/w157` | **Aplicado en producción** (21-sep-2026) — `20260921230906`. Cuatro fuentes ya: dos `CL` y dos `MX`. Corrida real: 22 eventos con coordenadas propias, precio en MXN y dirección. **Primera vez que dos fuentes comparten país y región**, así que la deduplicación entre fuentes distintas pasa de teórica a probable (abierta desde el 049) |
| 099 | La fuente `fever` en `event_sources` — tercera fuente mexicana, primera que no es ticketera local sino plataforma global. Habilita `sonopolisWeb/specs/w160` | **Aplicado en producción** (21-sep-2026) — `20260921233622`. Cinco fuentes: 2 `CL`, 3 `MX`. Esta fila es el catálogo **mexicano** de Fever: otro país sería otra fila con otro slug, porque el pipeline valida un país por fuente |
| 100 | Cada evento se cobra en la moneda de su país: tabla `paises_cobro` (`CL/CLP` activo, `MX/MXN` **inactivo**), `events.moneda` por trigger, `tickets.moneda` + `tickets.pais_cobro`, `precio_vigente_de` devuelve `moneda` y `se_vende`, la reserva se niega con `pais_sin_cobro`. Un evento con entradas no cambia de país. Habilita 101 y `sonopolisWeb/specs/w167` | **Aplicado en producción** (22-sep-2026) — `20260922120000`. 6 criterios verificados por SQL contra producción, en transacción revertida. **México no vende hasta que exista su cuenta de Mercado Pago** |
| 101 | Mercado Pago cobra con la cuenta y la moneda del país del evento: `_shared/cuentasMP.ts` (secrets por país), `create-preference` manda `currency_id` desde la base y 409 `pais_sin_cobro`, `webhook-mp` elige secreto y token por `?cuenta=`, `confirm-payment` pregunta con `tickets.pais_cobro` | **Desplegado en producción** (22-sep-2026) — las tres funciones `ACTIVE`. Falta que Victor corra una compra chilena real (cuesta dinero, no se automatiza) y, cuando exista la cuenta mexicana, una mexicana |
...
| 080 | `pais` en `events` y `venues` (`char(2)`, `NOT NULL`, sin `DEFAULT`), backfill a `CL` e índices `(pais, comienza_at)` / `(pais)`. Completa el spec 050: los eventos scrapeados ya sabían su país, los internos no | **Aplicado** (2026-09-08) — los 5 criterios verificados contra producción. ⚠️ **Se aplicó con la ventana del `NOT NULL` abierta, por decisión de Victor:** hoy **crear evento o local falla en producción** (`23502`) en la web y en la app móvil, porque ningún frontend manda el país todavía. Leer cartelera, comprar, canjear y editar no se tocan. La cierran, en orden: el W-114 de `sonopolisWeb` (que depende del W-111 y el W-109, ninguno implementado) y un spec FRONTEND de esta app, sin escribir, que estampe `pais` en `mapEventoToDB`/`mapVenueToDB` |
| 081 | La fila `puntoticket` en `event_sources` (`pais = CL`, `activa` por default), con `on conflict do nothing`. Sin esquema nuevo: las columnas que la fuente va a llenar (`direccion`, `lat`, `lng`, `precio_texto`, `genero`) existen desde el 049 | **Aplicado** (2026-09-08) — `20260908115505_spec_081_fuente_puntoticket.sql` corrida contra producción, los 4 criterios verificados (fila creada, `on conflict` reprobado corriendo el insert dos veces, `portaltickets` intacta). Habilita `sonopolisWeb/specs/w115` (adaptador de Puntoticket); sin la fila, el pipeline multifuente del W-110 salta la fuente con error |
| 082 | Cierre temporal obligatorio en `event_preventas`: `cierre_tipo` (`horas_antes`/`fecha`), `cierre_horas_antes`, `cierre_at`, con default recomendado `horas_antes`/3. Motivo: el 7-sep un fan compró en la puerta a precio de preventa — cupo y cierre manual no miran el reloj | **Aplicado** (8-sep-2026) — `20260908155200` en producción, 6 criterios verificados. La única preventa activa (la del incidente) recibió el default de 3 h |
| 083 | La preventa cierra por reloj en el cobro: `preventa_cierra_at(fila)` y `preventa_abierta(fila)` (columnas calculadas vía PostgREST), `precio_vigente_de` devuelve `cierra_at` y `_reservar_ticket_shared` usa el mismo predicado. Falla cerrado sin `comienza_at` | **Aplicado** (8-sep-2026) — `20260908155300` en producción, 7 criterios verificados con reserva real en transacción revertida; las 4 funciones remotas coinciden con el archivo. Desbloquea `sonopolisWeb` W-116 |
| 084 | La fila `ticketmaster` en `event_sources` (`pais = CL`) pero **`activa = false`**: el interruptor es la forma de que el adaptador exista sin scrapear un sitio cuyos Términos de Uso lo prohíben y cuyo `robots.txt` no tiene grupo `*`. Sin esquema nuevo | Escrito, sin aplicar (8-sep-2026) — habilita `sonopolisWeb/specs/w117`; encenderla es decisión de Victor, con un `update`, no una migración |
