...
| 101 | Mercado Pago cobra con la cuenta y la moneda del país del evento: `_shared/cuentasMP.ts` (secrets por país), `create-preference` manda `currency_id` desde la base y 409 `pais_sin_cobro`, `webhook-mp` elige secreto y token por `?cuenta=`, `confirm-payment` pregunta con `tickets.pais_cobro` | Propuesto (22-sep-2026) — depende del 100. Sin él, un pago de la cuenta mexicana nunca se confirmaría y `reconciliar-pagos` cancelaría una entrada pagada |
